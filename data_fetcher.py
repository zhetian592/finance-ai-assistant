...
def allocate_weights_by_score(df: pd.DataFrame, cash: float, top_n: int = 3,
    ...
    """
    根据行业得分分配资金
    df: 包含 sector, etf_code, total_score 等列
    cash: 总资金
    top_n: 选择前N个行业
    risk_level: high/medium/low，影响仓位比例
...
def generate_sector_report(df: pd.DataFrame, top_n: int = 5) -> str:
    ...
    html += "<td><th>行业</th><th>ETF代码</th><th>估值得分</th><th>资金得分</th><th>动量得分</th><th>情绪得分</th><th>综合得分</th></tr>"
    for _, row in df.head(top_n).iterrows():
        ...
        html += f"<tr {bg_class}><td>{row['sector']}</td><td>{row.get('etf_code', '')}</td><td>{row['valuation_score']:.1f}</td><td>{row['money_flow_score']:.1f}</td><td>{row['momentum_score']:.1f}</td><td>{row['sentiment_score']:.1f}</td><td><strong>{row['total_score']:.1f}</strong></td></tr>"
    html += "</table>"
    return html
