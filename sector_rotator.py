...
def allocate_weights_by_score(df: pd.DataFrame, cash: float, top_n: int = 3,
    ...
    参数:
        df: 包含 sector, etf_code, valuation_score, money_flow_score,
            momentum_score, sentiment_score, total_score, effective_count 等列
        cash: 总现金
        top_n: 选前N个行业
...
def generate_sector_report(df: pd.DataFrame, top_n: int = 5) -> str:
    ...
    for _, row in df_display.iterrows():
        ...
        html += f"<tr><td>{row['sector']}</td><td>{row['etf_code']}</td><td>{val}</td><td>{money}</td><td>{mom}</td><td>{sent}</td><td><strong>{total}</strong></td><td>{eff}</td></tr>"
        displayed += 1
    html += "</table>"
...
def generate_concentration_warning(recommendations: List[Dict]) -> str:
    """
    分析推荐组合的风格集中度，返回警告文本（若无风险返回空字符串）
    """
